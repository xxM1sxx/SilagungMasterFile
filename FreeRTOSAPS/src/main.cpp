...
const unsigned long NTP_SYNC_INTERVAL = 30000;
unsigned long lastTimeLog = 0;
const unsigned long TIME_LOG_INTERVAL = 5000;
const float MIN_FLOW_THRESHOLD = 0.10f;
const unsigned int NO_FLOW_STALL_SECONDS = 10; //berapa attemp ini maksudnya
...
void TaskMQTT(void* pv) {
  ...
  for (;;) {
    ...
    if (millis() - lastTimeLog >= TIME_LOG_INTERVAL) {
...
void TaskModbus(void* pv) {
  ...
  for (;;) {
      ...
      if (millis() - lastFlowAccum >= 1000) {
        ...
        if (millis() - lastPumpEnsure >= 1000) {
          ...
          if (!pon) {
            ...
            hmiNode.writeSingleCoil(HMI_BTN_ADDR + PUMP_BTN_INDEX, 1);
            if (!sc) {
        ...
        if (lowFlowSeconds >= (int)NO_FLOW_STALL_SECONDS || noProgressSeconds >= (int)NO_PROGRESS_STALL_SECONDS) {
          ...
          for (int i = 0; i < VALVE_COUNT; i++) {
            webButtonChange[i] = true;
            hmiNode.writeSingleCoil(HMI_BTN_ADDR + i, 0);
            vTaskDelay(pdMS_TO_TICKS(10));
          }
          // turn off pump via HMI button; TaskModbus will act on this state
          webButtonChange[VALVE_COUNT] = true;
          hmiNode.writeSingleCoil(HMI_BTN_ADDR + PUMP_BTN_INDEX, 0);
    ...
    // Read HMI buttons
    uint8_t res = hmiNode.readCoils(HMI_BTN_ADDR, VALVE_COUNT + 1);
    if (res == hmiNode.ku8MBSuccess) {
      ...
      if (pumpBtn != lastPumpBtnState) {
        ...
        if (webButtonChange[VALVE_COUNT]) {
    ...
    for(;;){
      ...
      if (!currentIrrigation.isActive && xQueueReceive(xIrrigationQueue, &job, pdMS_TO_TICKS(200)) == pdTRUE) {
        ...
        for (int k = 0; k < VALVE_COUNT; k++) {
          webButtonChange[k] = true;
          hmiNode.writeSingleCoil(HMI_BTN_ADDR + k, 0);
          vTaskDelay(pdMS_TO_TICKS(10));
        }
        webButtonChange[VALVE_COUNT] = true;
        hmiNode.writeSingleCoil(HMI_BTN_ADDR + PUMP_BTN_INDEX, 0);
        ...
        bool isAirOnly = currentIrrigation.irrigationType.equalsIgnoreCase("air");
        bool isAirNutrisi = currentIrrigation.irrigationType.equalsIgnoreCase("air_nutrisi") || currentIrrigation.irrigationType.equalsIgnoreCase("air+nutrisi");
        webButtonChange[0] = true;
        webButtonChange[1] = true;
        if (isAirOnly) {
          hmiNode.writeSingleCoil(HMI_BTN_ADDR + 0, 0);
          vTaskDelay(pdMS_TO_TICKS(10));
          hmiNode.writeSingleCoil(HMI_BTN_ADDR + 1, 1);
          controlRelayValve(0, false);
          vTaskDelay(pdMS_TO_TICKS(10));
          controlRelayValve(1, true);
        } else if (isAirNutrisi) {
          hmiNode.writeSingleCoil(HMI_BTN_ADDR + 0, 1);
          vTaskDelay(pdMS_TO_TICKS(10));
          hmiNode.writeSingleCoil(HMI_BTN_ADDR + 1, 1);
          controlRelayValve(0, true);
          vTaskDelay(pdMS_TO_TICKS(10));
          controlRelayValve(1, true);
        } else {
          hmiNode.writeSingleCoil(HMI_BTN_ADDR + 0, 0);
        ...
        for (int v = 3; v <= 5; v++) {
          ...
          hmiNode.writeSingleCoil(HMI_BTN_ADDR + idx, (v == currentIrrigation.landRelay) ? 1 : 0);
          vTaskDelay(pdMS_TO_TICKS(10));
        }
        {
          int landIdx = currentIrrigation.landRelay - 1;
          controlRelayValve(landIdx, true);
          vTaskDelay(pdMS_TO_TICKS(20));
        }
        // turn on pump via HMI button only; TaskModbus will act on this state
        webButtonChange[VALVE_COUNT] = true;
        hmiNode.writeSingleCoil(HMI_BTN_ADDR + PUMP_BTN_INDEX, 1);
        vTaskDelay(pdMS_TO_TICKS(10));
