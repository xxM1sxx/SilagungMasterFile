...
void writeEcNpkPhToHMI() {
    uint16_t ecVal = (uint16_t)ec;
    uint16_t nVal = nitrogen;
    uint16_t pVal = phosphorus;
    uint16_t kVal = potassium;
    uint16_t phVal = (uint16_t)roundf(phValue * 10.0f);
    writeHmiRegisterWithRetry(0x0004, ecVal);
    vTaskDelay(pdMS_TO_TICKS(hmiWriteDelayMs()));
    writeHmiRegisterWithRetry(0x0005, nVal);
    vTaskDelay(pdMS_TO_TICKS(hmiWriteDelayMs()));
    writeHmiRegisterWithRetry(0x0006, pVal);
    vTaskDelay(pdMS_TO_TICKS(hmiWriteDelayMs()));
    writeHmiRegisterWithRetry(0x0007, kVal);
    vTaskDelay(pdMS_TO_TICKS(hmiWriteDelayMs()));
    writeHmiRegisterWithRetry(0x0008, phVal);
    vTaskDelay(pdMS_TO_TICKS(hmiWriteDelayMs()));
}

void writePressureToHMI() {
    uint16_t p = (uint16_t)(pressure * 100); // Bar with 2 decimals
    writeHmiRegisterWithRetry(0x0002, p);
}
...
static bool readHmiRegister(uint16_t reg, uint16_t& value) {
  uint8_t res = 0xFF;
  esp_task_wdt_reset();
  lockModbus();
  res = hmiNode.readHoldingRegisters(reg, 1);
  if (res == hmiNode.ku8MBSuccess) {
    value = hmiNode.getResponseBuffer(0);
  }
  unlockModbus();
  esp_task_wdt_reset();
  return res == hmiNode.ku8MBSuccess;
}
...
static bool readRelayCoilState(uint16_t coil, bool& state) {
  ...
  esp_task_wdt_reset();
  lockModbus();
  res = relayNode.readCoils(coil, 1);
  if (res == relayNode.ku8MBSuccess) {
    word = relayNode.getResponseBuffer(0);
  }
  unlockModbus();
  esp_task_wdt_reset();
  if (res != relayNode.ku8MBSuccess) return false;
  state = (word & 0x01) != 0;
  return true;
}

static bool verifyRelayCoilState(uint16_t coil, bool expected, bool& actual) {
  for (int attempt = 0; attempt < 2; attempt++) {
    bool okRead = readRelayCoilState(coil, actual);
    if (okRead && actual == expected) return true;
    if (okRead) {
      writeCoilWithRetry(coil, expected);
...
bool readEcNpkPhSensor(ModbusMaster& node, float& ecValue, float& phOut, uint16_t& nOut, uint16_t& pOut, uint16_t& kOut, const char* sensorName) {
    uint8_t rc = 0xFF;
    ...
    for (uint8_t i = 0; i < READ_RETRY; i++) {
        ...
        rc = node.readHoldingRegisters(REG_EC, 5);
        esp_task_wdt_reset();
        if (rc == node.ku8MBSuccess) {
            ecValue = node.getResponseBuffer(0);
            phOut = node.getResponseBuffer(1) / 10.0f;
            nOut = node.getResponseBuffer(2);
            pOut = node.getResponseBuffer(3);
            kOut = node.getResponseBuffer(4);
            unlockModbus();
    ...
    if (consecutiveSensorErrors[1] >= MAX_CONSECUTIVE_ERRORS) {
        if (millis() - lastSensorFail[1] > SENSOR_FAIL_LOG_INTERVAL) {
            LOG_ERROR("%s read failure - error code: %u", sensorName, rc);
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, topic_control) == 0) {
    ...
    if (doc.containsKey("vfdHz") || doc.containsKey("vfd_hz") || doc.containsKey("vfdFrequency") || doc.containsKey("frequency") || doc.containsKey("pumpFrequency")) {
      ...
      if (ok) {
        uint16_t confirm = 0;
        bool okRead = readHmiRegister(10, confirm);
        if (!okRead || confirm != raw) {
          LOG_WARN("HMI VFD verify mismatch reg:10 want:%u got:%u", raw, okRead ? confirm : 0xFFFF);
        } else {
...
void TaskModbus(void* pv) {
  ...
  for (;;) {
      ...
      if (res == hmiNode.ku8MBSuccess) {
        ...
        for (int i = 0; i < VALVE_COUNT; i++) {
          ...
          if (btn != lastValveState[i]) {
            bool prev = lastValveState[i];
            lastValveState[i] = btn;
            if (!autoMode) {
              hadManualButtonChange = true;
              bool ok = controlRelayValveForceVerify(i, btn);
              if (ok) {
                writeHmiCoilWithRetry(HMI_LAMP_ADDR + i, btn);
                if (webButtonChange[i]) webButtonChange[i] = false;
              } else {
                writeHmiCoilWithRetry(HMI_BTN_ADDR + i, prev);
                writeHmiCoilWithRetry(HMI_LAMP_ADDR + i, prev);
                lastValveState[i] = prev;
              }
              needPublish = true;
            } else {
              xSemaphoreTake(xStateMutex, portMAX_DELAY);
        ...
        if (pumpBtn != lastPumpBtnState) {
          bool prevPump = lastPumpBtnState;
          lastPumpBtnState = pumpBtn;
          if (!autoMode) {
            hadManualButtonChange = true;
            bool ok = controlPumpRelay(pumpBtn);
            if (ok) {
              setPumpLamp(pumpBtn);
              if (webButtonChange[VALVE_COUNT]) webButtonChange[VALVE_COUNT] = false;
            } else {
              writeHmiCoilWithRetry(HMI_BTN_ADDR + PUMP_BTN_INDEX, prevPump);
              setPumpLamp(prevPump);
              lastPumpBtnState = prevPump;
            }
            needPublish = true;
          }
        }
    ...
    if (shouldPollSensors) {
      ...
      readEcNpkPhSensor(ecNode, ec, phValue, nitrogen, phosphorus, potassium, "NPK/PH Sensor");
      vTaskDelay(pdMS_TO_TICKS(sensorPollGapMs()));
    ...
    if (!currentIrrigation.isActive && millis() - lastSensorLog >= 10000) {
      ...
      if (airPct < 0) airPct = 0;
      if (airPct > 100) airPct = 100;
      LOG_INFO("Sensor readings - Flow:%.2f L/min EC:%.0f uS/cm pH:%.1f N:%u P:%u K:%u Pressure:%.2f Bar Nutri:%d%% Air:%d%%",
               waterFlow, ec, phValue, nitrogen, phosphorus, potassium, pressure, nutrPct, airPct);
